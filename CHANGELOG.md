...
- Shading a region means filling its own boundary curves by the even-odd rule, because crossing any of them toggles in and out of the region. One crossing of the region settles which way round that reads: the region holding the point at infinity — 74 of the 1060 in the tests — has to be filled the other way about.
- **The curves run through the middle of the faces they cross**, not alongside the strands, so each one reads as a loop drawn around a tangle. How far a curve may leave a strand comes from a ray cast across the face: it takes a fixed share of the room, so two curves sharing a face take their share from opposite sides and cannot meet. Three things had to be handled for that to come out looking like the paper's figures rather than like a fence:
  - A face narrows to nothing at a crossing, so a ray straight across finds no room and the curve gets pinched onto the very crossing it is meant to enclose. At a corner it is given the depth of the wedge it turns inside, measured along the bisector, instead.
  - A single tight spot scoops a notch out of the region. The offset now carries over from its neighbours at a bounded rate, which fills a narrow notch and leaves a wide one alone, and is then put back under the hard limit so nothing can reach a strand.
  - Offsetting a polyline folds it into a small loop at every corner that turns the wrong way. Cutting each loop out is what pulls the curve taut around the tangle instead of leaving it trailing whiskers at the crossings.
- The drawing is tested, not only the numbers: **140 drawn curves** are required to be simple, pairwise disjoint, and to meet D exactly at their own marked points. That last count is the strong one — it says a curve encircles its own tangle and never strays into a neighbouring one on the way round.
- The strands are bucketed into a grid so each ray only looks at the cells it can reach, and the overlay geometry is computed lazily, only when that view asks for it: a 28-crossing diagram takes about 19 ms, and nothing is paid for it while another view is up.
...
- The overlay geometry is tested too, not just the numbers: each of **1076** drawn curves is checked by winding number to separate the crossings of the region it bounds from the middle pieces of the edges of G that meet it. That is what fixes which side of the strands the curve is offset to, which is otherwise a coin flip. The shading is checked the same way — **9298** points across **1060** regions, each crossing required inside its region's fill and each edge of G required outside it.
- Service-worker cache bumped to `knot-lab-v29-tangle-loops`.
