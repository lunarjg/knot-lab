...
- **Clearance is the first principle, and it is checked rather than assumed.** Every move is swept against everything nearby and stopped short of the first thing it would have crossed. That alone is not enough: sweeping a point's path cannot see the two segments it drags, so a curve could slip past a strand without any point crossing one — which is how earlier versions drew regions that looked pleasingly fat and were not legal. The relaxation now stops every ten rounds and asks whether each curve is still simple, still disjoint from the others, and still meeting D exactly once per marked point; one that fails is put back to the last state that passed.
...
- Service-worker cache bumped to `knot-lab-v31-convex-loops`.
