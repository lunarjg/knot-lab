...
A curve passes through its marked points and otherwise runs through the middle of the faces it crosses, so it reads as a loop drawn around an alternating tangle rather than as a second copy of the strands. How far it may leave a strand is decided by how much room there is: a ray cast across the face finds the far side, and the curve takes a fixed share of it, so two curves sharing a face take their share from opposite sides and cannot meet. A face narrows to nothing at a crossing, where a ray finds no room at all, so at a corner the curve is given the depth of the wedge it turns inside instead — without that it gets pinched onto the very crossing it is supposed to enclose. A single tight spot would still scoop a notch out of the region, so the offset carries over from its neighbours at a bounded rate, which fills a narrow notch and leaves a wide one alone, and is then put back under the hard limit so nothing can reach a strand. Offsetting a polyline folds it into a small loop at every corner turning the wrong way; cutting those loops out is what pulls the curve taut around the tangle.
