...
- **New state view, Alt. decomposition**, beside all-A, all-B and Seifert, laid out the way the paper's Figure 8 is: the alternating regions shaded, the closed decomposition curves solid on top of them, and the middle piece of each nonalternating edge — an edge of G — a heavy bar between its two marked points, coloured + blue for an overstrand edge and − orange for an understrand edge. The diagram itself stays neutral, since the shading already says which region a strand is in.
- Shading a region means filling its own boundary curves by the even-odd rule, because crossing any of them toggles in and out of the region. One crossing of the region settles which way round that reads: the region holding the point at infinity has to be filled the other way about.
- **A region is a neighbourhood of its tangle, not a sleeve on it.** The curve is placed halfway across whatever corridor it is in, up to a target thickness: narrow corridor, the middle of it; open space, the target rather than a drift. Shortening would pull it straight back onto the strand it came from, so the thickness is set rather than pushed for.
- **Each curve is drawn as the shortest curve of its kind**: it must pass through its marked points, stay clear of every strand, and stay clear of every other curve, and length is minimised under those rules. That pulls it taut around its tangle and takes the slack and the notches out of it.
- **Taut is not convex**, since the curve leaves a dip behind wherever it comes round an obstacle. Once the shortening stops moving, the same step runs again with only its outward half kept: dips fill, nothing already bulging is pulled back in, and the curve walks out towards its own convex hull as far as the clearances allow. A region that stays long and thin is the diagram saying so — its tangle is a chain with other strands running between its crossings. Filling a dip is measured against a window of eleven points rather than against the two neighbours, since filling over two points at nine pixels apart still leaves a corner; and a point with nothing to fill now stays put rather than sliding along the curve, which only bunched points up and left the curve ragged.
- **A curve under tension is a string, so a dent left in one is a dent something is holding.** The relaxation walks each point downhill a step at a time and settles into whatever local minimum it is nearest, which leaves dents behind that nothing is holding at all. So the arcs are also pulled taut outright: the chord between two points of an arc replaces everything between them whenever that chord is one the curve is allowed to take — the same rules the relaxation obeys, asked globally rather than a step at a time. It is done at every hand-over down the ladder, once when the shortening ends, and once more after the convexifying, which puts a little slack back into the parts that were already taut. Over the test fixtures the number of dents with nothing holding them went from 101 to 10, and the deepest from 112px to 21px; on a 63-crossing diagram the curves came out 10% shorter with a third as many dents.
- A chord can miss every strand and still close over one, since a strand that loops inside the pocket it cuts off crosses neither the chord nor the arc — and a count of crossings would never notice. So the pocket is asked directly whether anything is inside it before the shortcut is taken.
- **Coarse before fine.** Shortening a curve is a heat flow, so a dent takes about as many rounds to fill as the square of its width in points — and at nine pixels a point the wide dents in a big diagram were never reached. A 63-crossing diagram still had grooves in it after 400 rounds. The curve is now pulled taut on a coarse copy of itself first, at four times the spacing with a quarter of the points and the same shapes sixteen times sooner, and the spacing is halved back down to the drawn one as it goes. Every rule holds at every level, since they are all written in pixels rather than in points, and a level that stops moving hands over straight away rather than spending its budget. Same curve, a fifth of the work: that 63-crossing diagram is now round in 3.8 s where 400 rounds at the drawn spacing took 10.7 s and still left grooves.
- **Whether it is still going anywhere is asked of the length it is minimising**, not of the furthest point. One point stuck against a strand held the old test open for ever, so on anything bigger than a few crossings the relaxation never reached its second phase and never came out convex at all — and every diagram paid for the whole budget whether it needed it or not. An eight-crossing diagram now settles and stops in 0.4 s.
- **The last raggedness is smoothed off.** Each point is placed against whichever strand happens to be nearest it, and where the nearest one changes from a point to the next the placement jumps. A dozen passes of plain smoothing take that off, under two rules that keep it from being another relaxation: no point may end up further than a clearance from where the relaxation left it, and the clearance is re-imposed after every move, so smoothing can never walk a curve into a strand. Across the test fixtures the tangent now swings through 4.6 half-turns on the way round a curve rather than 8.0 — a convex curve spends exactly 2, and every wiggle costs more.
- **Clearance is the first principle, and it is checked rather than assumed.** Every move is swept against everything nearby and stopped short of the first thing it would have crossed, and must additionally leave both of the segments it drags crossing the diagram exactly as often as they did before — a point's own path can miss a strand that one of its segments sweeps over. The relaxation then stops every five rounds and asks whether each curve is still simple, still disjoint from the others, and still meeting D exactly once per marked point; one that fails is put back to the last state that passed.
- **A bug in that check had been holding every region thin.** A segment sits in every grid cell it spans, so a range query can hand the same one back more than once — harmless when taking a nearest distance, wrong when counting crossings. The count came out high, every curve looked as though it had slipped, and every attempt to widen a region was reverted. Counting now goes through a stamp, and the curves move as they were meant to: on a 28-crossing diagram the mean gap between a curve and the nearest strand went from 10 to 28.
...
- **An alternating diagram was painted out entirely.** Its one region is the whole picture and nothing bounds it, so filling it opaquely left a blank canvas with the knot underneath — every alternating diagram, which is most of them. A region with nothing bounding it is now left alone.
- **No slice of the relaxation may be long enough to drop a frame.** Pulling the arcs taut, and the smoothing and assembly at the end, each ran whole inside a single slice, and a round was one slice: on a 63-crossing diagram that was three slices of 660 ms and a tail of 350 ms. The pull now hands back between arcs, the tail between passes, and a round between curves. The longest slice went from 662 ms to 80 ms.
- **The pull's reach is capped.** A chord is tested by sampling along it, so letting the reach run to the end of an arc made the pull quadratic in its length — unbounded work on a curve with a long open run. Capped at 120 points it is linear, and the curves come out the same: the same 10 dents with nothing holding them, the deepest still 21px.
- **The overlay hands back with a task rather than with a frame.** An animation frame comes round every sixteen milliseconds whatever it is given, so six milliseconds of work per frame left two thirds of the wait idle: once the slices were small enough to respect the budget, a 63-crossing diagram took 7.6 s instead of 4.0 s. It now posts itself a message instead — an ordinary task, with input and painting in between — and runs at the rate it can, with no task over 60 ms anywhere.
- Checking whether a curve is contained in another rejects on the bounding box first, so finding the outermost ones does not walk every curve against every other.
...
- **Auto-relax no longer looks hung in this view.** Relaxing the curves against the whole diagram is worth a moment once, but it was being redone on every frame of the run — 25 rebuilds for one press, which on an eight-crossing diagram took 13 seconds against 0.6 in the ordinary view, and far worse on a bigger one. Auto-relax now shows the plain diagram while it steps, exactly as a drag does, and the decomposition comes back when it stops: 1.1 seconds for the same eight crossings, 2.0 for twenty-eight. Anything thrown while stepping also ends the run, rather than breaking the chain of frames and leaving it stuck part-way with the button still offering to stop it.
- **Dragging a strand in this view no longer stutters.** Relaxing the curves is a few hundred milliseconds of arithmetic, and it was being done on the way to the screen: the drag itself was smooth, but letting go froze the page for a third of a second, every time. The relaxation is now a generator that gives the thread back between rounds, and the app drives it about six milliseconds per frame — so the diagram paints the moment the drag ends and the overlay arrives a little after, with no frame over 50 ms anywhere in the gesture (it was 346 ms). A run started while the strands are still moving is dropped rather than finished, and only one runs at a time.
- The arithmetic itself is a little under half what it was. Where the relaxation asks for the nearest point on a segment — millions of times a run — it now leaves the answer in two scratch numbers instead of returning a small object for each one, since collecting those cost more than the sums did; and the budget is 50 rounds rather than 90, which moves the curves by about a percent of their length and no more.
- Starting each run from the previous one's curves was tried and taken out: it is twice as quick, but the error accumulates over a chain of edits until curves are crossing strands. Every run starts clean.
- Strands and curves are bucketed into grids, the curve grid kept exact as points move, and the overlay geometry is computed lazily, only when that view asks for it; nothing is paid for it while another view is up.
- **New inspector section** with the nonalternating edge count, the number of curves, the number of alternating regions, the size of G, and g<sub>T</sub>(G). It also draws **G on its own**, as Figure 8 does beside the diagram: a disk per curve, an arc per nonalternating edge, arcs of a parallel class bowed apart and interleaved by sign so two of the same colour never sit side by side and read as one line.
