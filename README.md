...
A curve is drawn as the shortest curve of its kind. It has to pass through its marked points, it has to stay clear of every strand, and it has to stay clear of every other curve; length is then minimised under those rules, which is what makes it come out round — a loop pulled taut around an alternating tangle. Shortening on its own would collapse the curve onto the strands, so a gentle outward pressure balances it for part of the relaxation: the region grows until it runs out of room, and what the two together settle on is the roundest shape that fits.

The clearance rule is the first principle rather than an afterthought. Every move a point makes is swept against everything near it and stopped short of the first thing it would have crossed, so a point can never end up on the far side of a strand, whatever the other rules asked for — and a curve that cannot cross a strand cannot take in one belonging to a different tangle. Each curve's points are kept as the arcs between its marked points, which are re-spaced as the curve shortens; without that, points crowd onto a pin and the curve folds over its own marked point. Relaxation with a bounded step can still, very occasionally, walk a curve through itself, so the last state checked and found simple is kept and a curve that ties itself up is put back to it.
