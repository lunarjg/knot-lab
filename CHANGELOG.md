...
- Strands and curves are bucketed into grids, the curve grid kept exact as points move, and the overlay geometry is computed lazily, only when that view asks for it. An eight-crossing diagram takes about 0.4 s and a 28-crossing one about 1.1 s; nothing is paid for it while another view is up.
...
- Service-worker cache bumped to `knot-lab-v34-sliding-marks`.
