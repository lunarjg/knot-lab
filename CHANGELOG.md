...
- Strands and curves are bucketed into grids, the curve grid kept exact as points move, and the overlay geometry is computed lazily, only when that view asks for it. An eight-crossing diagram takes about half a second and a 28-crossing one about a second and a half; nothing is paid for it while another view is up, and a gesture that is still moving strands about shows the plain diagram until it is let go.
...
- Service-worker cache bumped to `knot-lab-v32-plump-loops`.
