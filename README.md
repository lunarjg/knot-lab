...
The view first tries a **circle around each alternating tangle**, using its crossings and alternating edges as the core. Long nonalternating connectors belong to the edges between tangles, so their midpoint no longer forces a tangle boundary into a long sleeve or groove. If a circle would cut a foreign strand or cross a connector more than once, the view tries progressively elongated ellipses.
...
Relaxing the curves is a few hundred milliseconds of arithmetic, which is long enough to be felt, so it is never done on the way to the screen. It is written as a generator that gives the thread back between curves, between rounds, and between the passes of the work at either end of a run, so no slice of it is long enough to drop a frame. The app takes about six milliseconds of it at a time and then hands back to the browser with a message to itself rather than with an animation frame — a frame comes round every sixteen milliseconds whatever it is given, so waiting for one would leave two thirds of the time idle. The diagram paints as soon as the gesture ends and the overlay arrives shortly after, rather than the page freezing at every release. A run started while the strands are still moving is dropped rather than finished, and only one ever runs at a time. Starting a run from the previous one's curves was tried and taken out — it is twice as quick, but the error accumulates over a chain of edits until curves are crossing strands, so every run starts clean.
...
Shading is a switch with a strength, and at full strength shading a region hides its tangle. The fill goes over the diagram rather than under it, in two passes — paper, which is what covers the tangle, and the tint over it, which is what gives the region its colour. Only the first carries the strength, so the colour stays the one it has always been and what changes is how much of the diagram shows through: hidden at 100%, the light wash the shading used to be down at 10%. What is left on the canvas is then G itself: a blob for every vertex and the nonalternating edge between them for every edge. Turn it off to see the diagram the regions are drawn on. The region holding the point at infinity is bounded by the curves that nothing else contains rather than by the ones assigned to it, since a curve belongs to the region whose tangle it encircles and a split diagram can leave the outer one with none of its own.

The signs are a setting too. Colouring each edge of G by its sign and writing + or &minus; beside it is an annotation on the edge rather than the edge itself, so it switches off on its own: the edge stays, drawn neutrally, and the glyphs go. It is drawn at the width of the strand it lies on, since it is a piece of the diagram rather than something laid over it. The graph figure in the inspector always shows the signs, since the way they alternate around a vertex is what makes a parallel class countable. The marked points themselves carry no dot — the two ends of a bar are exactly where they are.
