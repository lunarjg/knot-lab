...
Shading is a switch, and shading a region hides its tangle. The fill goes over the diagram rather than under it, and is opaque — paper first and the tint over it, so a shaded region reads exactly the colour it always did while the tangle inside is gone. What is left on the canvas is then G itself: a blob for every vertex and the nonalternating edge between them for every edge. Turn it off to see the diagram the regions are drawn on. The region holding the point at infinity is bounded by the curves that nothing else contains rather than by the ones assigned to it, since a curve belongs to the region whose tangle it encircles and a split diagram can leave the outer one with none of its own.
