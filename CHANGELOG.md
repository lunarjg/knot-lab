...
- **Each curve is drawn as the shortest curve of its kind**: it must pass through its marked points, stay clear of every strand, and stay clear of every other curve, and length is minimised under those rules. That is what makes it come out round — a loop pulled taut around an alternating tangle. Shortening alone would collapse it onto the strands, so a gentle outward pressure balances it for the first half of the relaxation: the region grows until it runs out of room, and the two together settle on the roundest shape that fits.
- **Clearance is the first principle, not an afterthought.** Every move is swept against everything nearby and stopped short of the first thing it would have crossed, so no step size can put a point on the far side of a strand — and a curve that cannot cross a strand cannot take in one belonging to another tangle.
- Two things had to be got right for the relaxation to stay well behaved. Each curve is kept as the arcs between its marked points and re-spaced as it shortens; without that, points crowd onto a pin and the curve folds over its own marked point. And the last state checked and found simple is kept, so a curve that does manage to tie itself up is put back to it rather than drawn knotted.
- The curves are stroked and filled as a quadratic spline through the segment midpoints, which rounds the polyline off without leaving the corners it cuts, so the clearances still hold.
...
- Strands and curves are bucketed into grids so each query only looks at the cells it can reach, and the overlay geometry is computed lazily, only when that view asks for it. An eight-crossing diagram takes about 50 ms and a 28-crossing one about 450 ms, nothing is paid for it while another view is up, and a gesture that is still moving strands about shows the plain diagram until it is let go.
...
- Service-worker cache bumped to `knot-lab-v30-taut-loops`.
