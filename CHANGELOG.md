...
- The relaxation is tested as something that can be run in slices: draining the generator in one go and stepping it 49 times must land on exactly the same curves, and no single slice may carry the whole run.
- Service-worker cache bumped to `knot-lab-v35-sliced-relaxation`.
